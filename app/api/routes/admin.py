...
@router.get("/dispatch-debug")
def dispatch_debug(
    _: Annotated[User, Depends(require_admin)],
    db: Annotated[Session, Depends(get_db)],
) -> dict:
    """Temporary: diagnose why some users see no work."""
    TARGET = 3
    ...
    pages_with_open_lines = db.execute(
        select(func.count(Page.id)).where(
    ...
    users = db.execute(select(User.id, User.email, User.display_name)).all()

    per_user = []
    for uid, email, name in users:
        done = db.execute(
            select(func.count(Transcription.id)).where(Transcription.user_id == uid)
        ).scalar_one()

        user_transcribed_subq = (
            select(Transcription.line_id)
            .where(Transcription.user_id == uid)
            .scalar_subquery()
        )
        eligible = db.execute(
        ...
        per_user.append({
            "email": email,
            "name": name,
            "transcriptions_done": done,
            "eligible_lines_remaining": eligible,
        })

    return {
        "approved_pages": approved_total,
        "approved_pages_with_open_lines": pages_with_open_lines,
        "target": TARGET,
        "users": per_user,
    }
