...
2. `transcription_count < settings.transcription_target` (default `1`, see `app/config.py`)
...
- **No consensus in the app.** The platform collects independent responses and exports them. Normalization, voting, and quality filtering happen in downstream scripts. With `transcription_target = 1` there is no redundant coverage to reconcile — each line gets exactly one contributor's transcription unless a volunteer over-collects. Raise the target back above 1 if downstream consensus/voting needs overlapping responses per line again.
